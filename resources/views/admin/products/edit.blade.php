...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-900">Sửa sản phẩm</h2>
        </div>
        ...
        <form action="{{ route('admin.products.update', $product) }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('name') border-red-500 @enderror" required>
                        @error('name') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
                    </div>
                    
                    <div>
                        <label for="slug" class="block text-sm font-medium text-gray-700 mb-2">Slug <span class="text-red-600">*</span></label>
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $product->slug) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('slug') border-red-500 @enderror" required>
                        @error('slug') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
                    </div>
                    
                    <div>
                        <label for="sku" class="block text-sm font-medium text-gray-700 mb-2">SKU <span class="text-red-600">*</span></label>
                        <input type="text" name="sku" id="sku" value="{{ old('sku', $product->sku) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('sku') border-red-500 @enderror" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="price" id="price" value="{{ old('price', $product->price) }}" step="0.01" min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('price') border-red-500 @enderror" required>
            ...
            <div class="border-t pt-6">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="short_description" id="short_description" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg">{{ old('short_description', $product->short_description) }}</textarea>
                </div>
            ...
            <div class="border-t pt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="unit" id="unit" value="{{ old('unit', $product->unit) }}" placeholder="VD: m², cái, ..." class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                    </div>
                    
                    <div>
                        <label for="material" class="block text-sm font-medium text-gray-700 mb-2">Vật liệu</label>
                        <input type="text" name="material" id="material" value="{{ old('material', $product->material) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                    </div>
                    
                    <div>
                        <label for="size" class="block text-sm font-medium text-gray-700 mb-2">Kích thước</label>
                        <input type="text" name="size" id="size" value="{{ old('size', $product->size) }}" placeholder="VD: 60x60cm" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                    </div>
                    
                    <div>
                        <label for="surface_type" class="block text-sm font-medium text-gray-700 mb-2">Loại bề mặt</label>
                        <input type="text" name="surface_type" id="surface_type" value="{{ old('surface_type', $product->surface_type) }}" placeholder="VD: Bóng, mờ, ..." class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                    </div>
                    
                    <div>
                        <label for="water_absorption" class="block text-sm font-medium text-gray-700 mb-2">Độ hút nước</label>
                        <input type="text" name="water_absorption" id="water_absorption" value="{{ old('water_absorption', $product->water_absorption) }}" placeholder="VD: < 0.5%" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                    </div>
                    
                    <div>
                        <label for="hardness" class="block text-sm font-medium text-gray-700 mb-2">Độ cứng</label>
                        <input type="text" name="hardness" id="hardness" value="{{ old('hardness', $product->hardness) }}" placeholder="VD: PEI 4" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                    </div>
                    
                    <div>
                        <label for="glaze_technology" class="block text-sm font-medium text-gray-700 mb-2">Công nghệ phủ men</label>
                        <input type="text" name="glaze_technology" id="glaze_technology" value="{{ old('glaze_technology', $product->glaze_technology) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                    </div>
                    
                    <div>
                        <label for="sort_order" class="block text-sm font-medium text-gray-700 mb-2">Thứ tự sắp xếp</label>
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $product->sort_order) }}" min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                    </div>
                </div>
            </div>
            ...
            <div class="border-t pt-6 flex gap-4">
                <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium">Cập nhật sản phẩm</button>
                <a href="{{ route('admin.products.images.edit', $product->id) }}" class="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium">📸 Quản lý ảnh</a>
